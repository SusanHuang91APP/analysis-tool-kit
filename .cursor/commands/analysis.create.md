...
- **參數說明**:
  ...
  - `--name`: AI 根據原始碼分析後建議的檔案名稱，**格式需根據 `type` 決定**:
    - `feature`: 使用描述性**中文**名稱 (例如: `工具列`)。
    - `api`, `helper`, `request-pipeline`, `component`: 使用 **PascalCase** 英文名稱 (例如: `SalePageService`, `TrackingUtility`)。
    - `server`, `client`: 此參數不影響最終檔名。
...
從腳本的 JSON 輸出中，獲取新建立檔案的路徑 (`FILE_PATH`) 和檔名 (`FILE_NAME`)，用於後續的元數據填充和回報。

### 步驟 4: 填充初始元數據

[ **注意**: 這是 `create` 指令中唯一允許的檔案編輯操作。]

1.  **讀取新檔案**: 根據腳本回傳的 `FILE_PATH`，讀取剛剛建立的分析檔案。

2.  **準備元數據**:
    -   將 `[source-files...]` 的路徑整理成 Markdown 表格格式。

3.  **填入資訊**:
    -   使用 `edit_file`，將「分析檔案資訊」區塊的 `[待補充：被分析的原始檔案完整路徑]` 佔位符替換為實際的原始碼檔案路徑表格。

### 步驟 5: 回報完成

**1. 驗證產出**:

- 根據腳本回傳的 `FILE_PATH`，確認檔案已成功建立並填入元數據。
...
2.  **有限的檔案編輯權限**: AI **嚴禁**在此指令中使用 `edit_file` 來修改任何分析檔案或 `overview.md`，**唯一的例外**是在檔案建立後，立即填充「分析檔案資訊」區塊的原始碼路徑。除此之外，不允許任何其他內容修改。

3.  **參數化呼叫**: 必須使用 `--type` 和 `--name` 參數，將使用者確認的類型和 AI 根據規則建議的名稱準確傳遞給腳本。
